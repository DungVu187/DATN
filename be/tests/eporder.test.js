...
describe('EpOrder API', () => {
  ...
  it('lets staff with only eporder.create edit their own draft but not other orders', async () => {
    const otherOrder = await createEpOrder({ orderName: 'Other draft' });
    const agent = await createAdminAgent({
      phone: '0933000007',
      role: 'staff',
      functions: ['eporder_management'],
    ...
    const otherEditRes = await agent
      .put(`/eporders/orders/${otherOrder._id}/name`)
  ...
  it('requires eporder.create or eporder.edit to upload and eporder.edit to delete invoice images', async () => {
    const agent = await createAdminAgent({
      phone: '0933000010',
      role: 'staff',
      functions: ['eporder_management'],
    ...
    expect(uploadRes.body.message).toBe('Access denied, missing one of permissions: eporder.create, eporder.edit');
